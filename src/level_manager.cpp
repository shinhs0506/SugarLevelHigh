...
void LevelManager::init(GLFWwindow *window) {
  this->window = window;
  this->main_camera =
      registry.cameras.entities[0]; // currently we only have one camera
...
void LevelManager::init_data(int level) {
  ...
  for (auto &player_data : reload_manager.get_player_data()) {
    gingerbread_advanced_attack.current_cooldown =
        player_data.advanced_attack_cooldown;
    AttackArsenal ginerbread_arsenal = {gingerbread_basic_attack,
                                        gingerbread_advanced_attack};
    Entity player =
        createPlayer(player_data.pos, player_data.size, player_data.health,
  ...
  for (auto &enemy_data : reload_manager.get_enemy_data()) {
    gumball_advanced_attack.current_cooldown =
        enemy_data.advanced_attack_cooldown;
    AttackArsenal gumball_arsenal = {gumball_basic_attack,
                                     gumball_advanced_attack};
    Entity enemy =
        createEnemy(enemy_data.pos, enemy_data.size, enemy_data.health,
...
void LevelManager::load_level(int level) {
  ...
  basic_attack_button =
      createButton(vec2(100, 300), vec2(50, 50), mock_basic_attack_callback);
  advanced_attack_button =
      createButton(vec2(100, 375), vec2(50, 50), mock_advanced_attack_callback);
...
void LevelManager::save_level_data() { reload_manager.save(curr_level); }
...
void LevelManager::remove_character(Entity entity) {
  ...
  // this is to ensure turn correctly ends the current active chracter died in
  // movement state
...
void LevelManager::update_curr_level_data() {
  ...
  CameraData camera_data{camera_motion.position,
  ...
  for (auto &player : registry.playables.entities) {
    ...
    PlayerData pd{player_motion.position, player_motion.scale,
                  player_health.cur_health, player_energy.cur_energy,
  ...
  for (auto &enemy : registry.enemies.entities) {
    ...
    EnemyData ed{enemy_motion.position, enemy_motion.scale,
                 enemy_health.cur_health, enemy_energy.cur_energy,
  ...
  for (auto &terrain : registry.terrains.entities) {
    ...
    TerrainData td{terrain_motion.position, terrain_motion.scale};
  ...
  for (auto &ladder : registry.climbables.entities) {
    ...
    LadderData ld{ladder_motion.position, ladder_motion.scale};
...
bool LevelManager::step(float elapsed_ms) {
  ...
  for (uint i = 0; i < registry.healths.size(); i++) {
    ...
    if (health.dead) {
      ...
      if (registry.playables.has(entity)) {
        ...
        remove_character(entity);
        removeEnemy(entity);
      } else if (registry.terrains.has(entity) &&
                 registry.terrains.get(entity).breakable) {
  ...
  bool only_player_left =
      registry.playables.size() == registry.initiatives.size();
    ...
    for (uint i = 0; i < registry.hitEffects.size(); i++) {
      ...
      if (effect.ttl_ms < 0) {
        ...
        if (registry.healths.has(entity) &&
            registry.healths.get(entity).cur_health < epsilon) {
...
void LevelManager::update_healthbar_len_color(Entity entity) {
  ...
  healthBar_motion.scale = {
      healthBar_motion.scale.x * (health.cur_health / health.max_health), 10};
...
void LevelManager::handle_collisions() {
  ...
  for (uint i = 0; i < registry.collisions.size(); i++) {
    ...
    if (registry.attackObjects.has(entity)) {
      ...
      // only be able to deal damage if two entities are different type // (not
      // both playables or enemies)
      bool different_team =
          (is_player_attack && !registry.playables.has(other_entity)) ||
          (!is_player_attack && !registry.enemies.has(other_entity));

      // check attacked set to make sure only deal damage once
      bool attacked =
          attack.attacked.find(other_entity) != attack.attacked.end();

      if (damagable && different_team && !attacked) {

        // If attack object is a projectile, reduce its ttl so that it dies
        // shortly on collision Shortly because we want abit of visuals that
        // contact is made
        if (registry.projectiles.has(entity)) {
          attack.ttl_ms = 10.0f;
        }
        // If attack was made to unbreakable terrain, shortcircut and do not do
        // hit effect/damage
        if (registry.terrains.has(other_entity) &&
            !registry.terrains.get(other_entity).breakable) {
          continue;
        }
        ...
        health.cur_health =
            clamp(health.cur_health - attack.damage, 0.f, FLT_MAX);
        attack.attacked.insert(other_entity);

        // change health bar length
        update_healthbar_len_color(other_entity);

        createHitEffect(other_entity,
                        200); // this ttl should be less then attack object ttl
      }
    }
  }
}

bool LevelManager::is_over() { return is_level_over; }
...
void LevelManager::on_key(int key, int scancode, int action, int mod) {
  ...
  if (action == GLFW_PRESS) {
    switch (key) {
    case GLFW_KEY_LEFT:
      update_camera(vec2(-CAM_MOVE_SPEED, 0));
      break;
    case GLFW_KEY_RIGHT:
      update_camera(vec2(CAM_MOVE_SPEED, 0));
      break;
    case GLFW_KEY_UP:
      update_camera(vec2(0, -CAM_MOVE_SPEED));
      break;
    case GLFW_KEY_DOWN:
      update_camera(vec2(0, CAM_MOVE_SPEED));
      break;
...
void LevelManager::move_to_state(LevelState next_state) {
  ...
  case LevelState::PREPARE:
    ...
    assert(this->current_level_state == LevelState::EVALUATION);
    break;

  case LevelState::PLAYER_TURN:
    std::cout << "moving to player's state" << std::endl;
    assert(this->current_level_state == LevelState::PREPARE);
    break;

  case LevelState::ENEMY_TURN:
    std::cout << "moving to enemy move state, AI is handling enemy movement"
              << std::endl;
    assert(this->current_level_state == LevelState::PREPARE);
    break;

  case LevelState::EVALUATION:
    std::cout << "moving to evaluation state, calculating damages..."
              << std::endl;
    assert(this->current_level_state == LevelState::PLAYER_TURN ||
           this->current_level_state == LevelState::ENEMY_TURN);
    break;
