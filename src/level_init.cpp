...
Entity createButton(vec2 pos, vec2 size, void (*on_click)())
{
	...
	// Setting initial motion values - should prob do this some other way since buttons don't have angle/velocity 
	Motion& motion = registry.motions.emplace(entity);
	motion.position = pos;
	motion.angle = 0.f;
	motion.velocity = { 0.f, 0.f };
	...
	registry.buttons.insert(entity, button);
...
Entity createMouseEvent(vec2 pos)
{
	...
	motion.angle = 0.f;
	motion.velocity = { 0.f, 0.f };
	motion.scale = { 10.f, 10.f }; // arbitrary value to allow collisions - what should this be?
